...
// ----------------------------------------------------------------------- //
const getPageProposer = async (req, res) => {
    res.render("proposer");
  },
  // FILTRE DES ENIGMES
  ...
  postEnigme = async (req, res) => {
    ...
    const insertEnigme = await db.query(
      ...
      { titre, difficulty, content, solus }
    );

    const [newEnigme] = await db.query(
      `SELECT * FROM enigme WHERE id_enigme = ${insertEnigme.insertId}`
    );
  ...
  getEnigmeId = async (req, res) => {
    // console.log("Params log :", req.params);
    const { id } = req.params;
