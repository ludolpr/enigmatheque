const 
    rdnEnigme = async function (res, id, flash) {
        ...
        if(!enigme[0]){
            ...
            enigmePost = await db.query(`SELECT commentaires.*, users.name FROM commentaires INNER JOIN users ON commentaires.user_id=users.id WHERE commentaires.article_id=${articles[0].id}`);
        }
